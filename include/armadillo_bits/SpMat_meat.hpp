...
SpMat<eT>::operator=(const SpSubview<eT>& X)
  ...
  if(alias)
    ...
    //   uword count = 0;
    //   
    //   for(uword m_col = 0; m_col < X.m.n_cols; ++m_col)
    //     {
    //     const eT val = X.m.at(X.aux_row1, m_col);  // use binary search
    //     
    //     if(val != eT(0))
    //       {
    //       access::rw(row_indices[count]) = 0;
    //       access::rw(values[count])      = val;
    //       ++access::rw(col_ptrs[m_col + 1]);
    //       count++;
    //       }
    //     }
    //   }
    ...
    //   typename SpMat<eT>::const_row_iterator m_it = X.m.begin_row(X.aux_row1);
    //   
    //   for(uword count=0; count < X.n_nonzero; ++count)
    //     {
    //     access::rw(row_indices[count]) = 0;
    //     access::rw(values[count]) = (*m_it);
    //     ++access::rw(col_ptrs[m_it.col() + 1]);
