...
inline
void
spglue_max_mixed::apply(SpMat<typename eT_promoter<T1,T2>::eT>& out, const mtSpGlue<typename eT_promoter<T1,T2>::eT, T1, T2, spglue_max_mixed>& expr)
  {
  arma_extra_debug_sigprint();
...
inline
void
spglue_max_mixed::dense_sparse_max(Mat< typename promote_type<typename T1::elem_type, typename T2::elem_type >::result>& out, const T1& X, const T2& Y)
  {
  arma_extra_debug_sigprint();
  ...
  arma_debug_assert_same_size( out.n_rows, out.n_cols, pb.get_n_rows(), pb.get_n_cols(), "element-wise max" );
...
// Get max of non-complex elements.
template<typename eT1, typename eT2>
inline
typename enable_if2<is_cx<eT1>::no && is_cx<eT2>::no, typename promote_type<eT1, eT2>::result>::result
spglue_max_mixed::elem_max(const eT1& a, const eT2& b)
  {
  typedef typename promote_type<eT1, eT2>::result out_eT;
  return std::max((out_eT) a, (out_eT) b);
  }
