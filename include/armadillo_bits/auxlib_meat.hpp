...
auxlib::qr_pivot(Mat<eT>& Q, Mat<eT>& R, Col<uword>& P, const Base<eT,T1>& X)
  ...
  arma_extra_debug_sigprint();
    ...
    for(uword col=0; col < R_n_cols; ++col)
      ...
      P.at(col) = jpvt[col] - 1;  // NOTE: JPVT index is 1-based, so subtract 1
      }
...
inline
bool
auxlib::qr_pivot(Mat< std::complex<T> >& Q, Mat< std::complex<T> >& R, Col<uword>& P, const Base<std::complex<T>,T1>& X)
  {
  arma_extra_debug_sigprint();
